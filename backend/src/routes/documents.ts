...
router.post('/upload', requireAuth, (req: AuthRequest, res, next) => {
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      console.error('POST /documents/upload error:', routeErr)
...
router.get('/', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const docs = settledDocs.map((r, i) =>
      r.status === 'fulfilled'
        ? r.value
        : {
            id:             result.rows[i].id,
            file_name:      result.rows[i].file_name,
            mime_type:      result.rows[i].mime_type,
            triage_case_id: result.rows[i].triage_case_id,
